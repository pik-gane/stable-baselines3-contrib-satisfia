...
def test_offpolicy_multi_env(model_class):
    ...
    if model_class in [TQC]:
        env_id = "Pendulum-v1"
        policy_kwargs.update(n_critics=1)
    else:
        env_id = "CartPole-v1"
        if model_class == ARDQN:
            kwargs.update(initial_aspiration=10)
...
def test_ep_buffers_stats_window_size(algo, stats_window_size):
    ...
    kwargs = dict(stats_window_size=stats_window_size)
    if algo == ARDQN:
        kwargs.update(initial_aspiration=0.0)
    model = algo("MlpPolicy", "CartPole-v1", **kwargs)
...
def test_multi_env_ardqn(env_id):
    ...
    # env = gym.make(env_id)
    model = ARDQN("MlpPolicy", env, learning_starts=0, initial_aspiration=0.0)
    model.learn(
        total_timesteps=100,
    )
    last_obs = env.reset()
