...
from sb3_contrib.ar_ddqn.policies import ArDDQNPolicy, CnnPolicyAr, MlpPolicy, MultiInputPolicyAr, QNetwork

SelfAR_DDQN = TypeVar("SelfAR_DDQN", bound="AR_DDQN")
...
class ArDDQN(DQN):
    ...
    :param initial_aspiration: The inital aspiration of the agent i.e the desired reward over a run
    ...
    policy_aliases: Dict[str, Type[BasePolicy]] = {
        "MlpPolicy": MlpPolicy,
        "CnnPolicy": CnnPolicyAr,
        "MultiInputPolicy": MultiInputPolicyAr,
    }
    ...
    def __init__(
        self,
        policy: Union[str, Type[ArDDQNPolicy]],
        ...
        super().__init__(
            ...
            train_freq,
            gradient_steps,
            replay_buffer_class=DictReplayBuffer,  # We need dict because we want to store lambdas
    ...
    def _create_aliases(self) -> None:
        # Overrides the DQN one to add qmax and qmin
    ...
    def rescale_aspiration(self, obs_t, a_t, obs_t1) -> None:
        """
        ...
        """
        # todo: try different version of it. It seems weird to use the target network
        #  during the exploitation phase
        lambda_t1 = (self.q_net(obs_t)[a_t] - self.qmin_net_target(obs_t)[a_t]) / (
            self.qmax_net_target(obs_t)[a_t] - self.qmin_net_target(obs_t)[a_t]
        )
        self.policy.aspiration = (1 - lambda_t1) * self.q_net_target(obs_t1).min() + lambda_t1 * self.q_net_target(
            obs_t1
        ).max()
